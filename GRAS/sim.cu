...
void synapses_kernel2(const bool *neuron_has_spike,     // array of bools -- is neuron has spike or not
                          ...
                          float *neuron_g_exc,
                          float *neuron_g_inh,
                          const int syn_number,
                          const int *nrn_syn_start,
                          const int *nrn_syn_end,
                          int *nrn_spike_waits,
                          const int nrn_num,
                          int iter,
                          const bool *nrn_has_posts,
        ...
        if (nrn_has_posts[tid]) {
            if (neuron_has_spike[tid]) {
                for (post_id = blockIdx.y * blockDim.y + threadIdx.y; post_id < nrn_syn_num[tid]; post_id += blockDim.y * gridDim.y) {
                    id = nrn_syn_start[tid] + post_id;
                    if (synapses_delay_timer[id] == -1) {
                        synapses_delay_timer[id] = synapses_delay[id];
            ...
            if (nrn_spike_waits[tid] > 0) {
                for (post_id = blockIdx.y * blockDim.y + threadIdx.y; post_id < nrn_syn_num[tid]; post_id += blockDim.y * gridDim.y) {
                    id = nrn_syn_start[tid] + post_id;
                    if (synapses_delay_timer[id] == 0) {
                        if (synapses_weight[id] >= 0) {
                            atomicAdd(&neuron_g_exc[synapses_post_nrn_id[id]], synapses_weight[id]);
                        } else {
                            atomicAdd(&neuron_g_inh[synapses_post_nrn_id[id]], -synapses_weight[id]);
                        }
                        atomicAdd(&nrn_spike_waits[tid], -1);
                        synapses_delay_timer[id] = -1;
                    }
                    else if (synapses_delay_timer[id] > 0) {
                        synapses_delay_timer[id]--;
                    }
                }
            }
        }
...
void simulate(int cms, int ees, int inh, int ped, int ht5, int save_all, int itest) {
    ...
    auto *nrn_synapses_end = init_cpu_arr<int>(neurons_number, 0);
...
//    for (int i = 0; i < syn_id; i++) {
//        cout << i << " " << synapses_pre_nrn_id[i] << " " << synapses_post_nrn_id[i] << endl;
//    }

    nrn_synapses_start[0] = 0;

    for (int i = 1; i < neurons_number; i++) {
...
//    exit(-1);
    ...
    auto *gpu_nrn_syn_end = init_gpu_arr<int>(nrn_synapses_end, neurons_number);
    auto *gpu_nrn_wait_spike = init_gpu_arr<int>(nrn_count_of_wait_spike, neurons_number);
    auto *gpu_has_post_nrns = init_gpu_arr<bool>(nrn_has_post_neurons, neurons_number);
