...
void neurons_kernel(const float *C_m,
	...
	if (tid < neurons_number) {
		...
		if (activated_C_ == 0 && early_activated_C_ == 0 && 2276 <= tid && tid <= 2471 && (sim_iter % 10 == 0)) {
			...
			has_spike[2276 + static_cast<int>(196 * curand_uniform(&localState))] = true;
		}
		// Skin stimulations
		if (activated_C_ == 1) {
			...
			// CV5
			if (tid == 125 && begin_C_spiking[5] < shifted_sim_iter && shifted_sim_iter < end_C_spiking[5] && curand_uniform(&localState) >= 0.5) {
				has_spike[tid] = true;
			}
		}
...
void connect_one_to_all(const Group& pre_neurons,
    ...
    if (strong_distr) {
        normal_distribution<float> delay_distr(syn_delay, syn_delay / 2.5);
        normal_distribution<float> weight_distr(weight, weight / 5);
...
void connect_fixed_outdegree(const Group& pre_neurons,
	...
	if (strong_distr) {
        normal_distribution<float> delay_distr_gen(syn_delay, syn_delay / 2.5);
        normal_distribution<float> weight_distr_gen(syn_weight, syn_weight / 5);
        for (unsigned int pre_id = pre_neurons.id_start; pre_id <= pre_neurons.id_end; pre_id++) {
            for (int i = 0; i < outdegree; i++) {
                int rand_post_id = id_distr(generator);
                float syn_delay_distr = delay_distr_gen(generator);
                if (syn_delay_distr <= 0.05) {
                    syn_delay_distr = 0.05;
                }
...
void init_network(float inh_coef, int pedal, int has5ht) {
	...
	Group CV4_1 = form_group("CV4_1", 1);
    Group CV4_2 = form_group("CV4_2", 1);
	...
	/// OM_1
	Group OM1_0 = form_group("OM1_0", 10);
	Group OM1_1 = form_group("OM1_1", 10);
	Group OM1_2_E = form_group("OM1_2_E", 10);
	Group OM1_2_F = form_group("OM1_2_F", 10);
	Group OM1_3 = form_group("OM1_3", 10);
	...
	/// OM_4
	Group OM4_0_1 = form_group("OM4_0_1");
	Group OM4_1_1 = form_group("OM4_1_1");
	Group OM4_2_E_1 = form_group("OM4_2_E_1");
	Group OM4_2_F_1 = form_group("OM4_2_F_1");
	Group OM4_3_1 = form_group("OM4_3_1");
	/////////////////////////////////////
    Group OM4_0_2 = form_group("OM4_0_2");
    Group OM4_1_2 = form_group("OM4_1_2");
    Group OM4_2_E_2 = form_group("OM4_2_E_2");
    Group OM4_2_F_2 = form_group("OM4_2_F_2");
    Group OM4_3_2 = form_group("OM4_3_2");
	...
	/// connectomes
	connect_fixed_outdegree(EES, E1, 1, 500, syn_outdegree, false, true);
	connect_fixed_outdegree(E1, E2, 1, 200, syn_outdegree, false, true);
	connect_fixed_outdegree(E2, E3, 1, 200, syn_outdegree, false, true);
	connect_fixed_outdegree(E3, E4, 1, 200, syn_outdegree, false, true);
	connect_fixed_outdegree(E4, E5, 1, 200, syn_outdegree, false, true);

	connect_one_to_all(CV1, iIP_E, 0.5, 50);
	connect_one_to_all(CV2, iIP_E, 0.5, 50);
	connect_one_to_all(CV3, iIP_E, 0.5, 50);
	connect_one_to_all(CV4_1, iIP_E, 0.5, 50);
	connect_one_to_all(CV4_2, iIP_E, 0.5, 50);
	...
	// input from sensory
	connect_one_to_all(CV1, OM1_0, 0.5, 10 * quadru_coef * sero_coef, false);
    connect_one_to_all(CV1, OM1_0, 0.5, 10 * quadru_coef * sero_coef, false);
	connect_one_to_all(CV2, OM1_0, 0.5, 10 * quadru_coef * sero_coef, false);
    connect_one_to_all(CV2, OM1_0, 0.5, 10 * quadru_coef * sero_coef, false);
	// [inhibition]
	connect_one_to_all(CV3, OM1_3, 1, 80);
    connect_one_to_all(CV3, OM1_3, 1, 80);
	connect_one_to_all(CV4_1, OM1_3, 1, 80);
    connect_one_to_all(CV4_2, OM1_3, 1, 80);
	...
	connect_fixed_outdegree(E2, OM2_0, 2, 6);
	...
	// [inhibition]
	connect_one_to_all(CV4_1, OM2_3, 1, 80);
    connect_one_to_all(CV4_2, OM2_3, 1, 80);
    ...
    connect_fixed_outdegree(OM2_2_F, OM3_2_F, 4, 30);
    // output to IP
	connect_fixed_outdegree(OM2_2_E, eIP_E, 2, 8 / 2, neurons_in_ip); // 5
	connect_fixed_outdegree(OM2_2_F, eIP_F, 4, 5 / 2, neurons_in_ip);

	/// OM 3
	// input from EES group 3
	connect_fixed_outdegree(E3, OM3_0, 1, 6);
    connect_fixed_outdegree(E3, OM3_0, 1, 6);
	// input from sensory [CV]
	connect_one_to_all(CV3, OM3_0, 0.5, 10.5 * quadru_coef * sero_coef);
	connect_one_to_all(CV4_1, OM3_0, 0.5, 10.5 * quadru_coef * sero_coef);
    connect_one_to_all(CV4_2, OM3_0, 0.5, 10.5 * quadru_coef * sero_coef);
	// [inhibition]
	...
	// output to OM3
	connect_fixed_outdegree(OM3_2_F, OM4_2_F_1, 4, 30);
    connect_fixed_outdegree(OM3_2_F, OM4_2_F_2, 4, 30);
    // output to IP
	...
	connect_fixed_outdegree(E4, OM4_0_1, 2, 7);
    connect_fixed_outdegree(E4, OM4_0_2, 2, 7);
    // input from sensory [CV]
	connect_one_to_all(CV4_1, OM4_0_1, 0.5, 10.5 * quadru_coef * sero_coef);
    connect_one_to_all(CV4_2, OM4_0_2, 0.5, 10.5 * quadru_coef * sero_coef);
	connect_one_to_all(CV5, OM4_0_1, 0.5, 10.5 * quadru_coef * sero_coef);
    connect_one_to_all(CV5, OM4_0_2, 0.5, 10.5 * quadru_coef * sero_coef);
    // input from sensory [CD]
	connect_one_to_all(CD4, OM4_0_1, 1, 11);
	connect_one_to_all(CD5, OM4_0_1, 1, 11);
    connect_one_to_all(CD4, OM4_0_2, 1, 11);
    connect_one_to_all(CD5, OM4_0_2, 1, 11);
	// inner connectomes
	connect_fixed_outdegree(OM4_0_1, OM4_1_1, 3, 50);
	connect_fixed_outdegree(OM4_1_1, OM4_2_E_1, 1, 25);
	connect_fixed_outdegree(OM4_1_1, OM4_2_F_1, 1, 23);
	connect_fixed_outdegree(OM4_1_1, OM4_3_1, 1, 3);
	connect_fixed_outdegree(OM4_2_E_1, OM4_1_1, 2.5, 25);
	connect_fixed_outdegree(OM4_2_F_1, OM4_1_1, 2.5, 20);
	connect_fixed_outdegree(OM4_2_E_1, OM4_3_1, 1, 3);
	connect_fixed_outdegree(OM4_2_F_1, OM4_3_1, 1, 3);
	connect_fixed_outdegree(OM4_3_1, OM4_1_1, 1, -70 * inh_coef);
	connect_fixed_outdegree(OM4_3_1, OM4_2_E_1, 1, -70 * inh_coef);
	connect_fixed_outdegree(OM4_3_1, OM4_2_F_1, 1, -3 * inh_coef);
	//////////////////////////////////////////////////////////////
    connect_fixed_outdegree(OM4_0_2, OM4_1_2, 3, 50);
    connect_fixed_outdegree(OM4_1_2, OM4_2_E_2, 1, 25);
    connect_fixed_outdegree(OM4_1_2, OM4_2_F_2, 1, 23);
    connect_fixed_outdegree(OM4_1_2, OM4_3_2, 1, 3);
    connect_fixed_outdegree(OM4_2_E_2, OM4_1_2, 2.5, 25);
    connect_fixed_outdegree(OM4_2_F_2, OM4_1_2, 2.5, 20);
    connect_fixed_outdegree(OM4_2_E_2, OM4_3_2, 1, 3);
    connect_fixed_outdegree(OM4_2_F_2, OM4_3_2, 1, 3);
    connect_fixed_outdegree(OM4_3_2, OM4_1_2, 1, -70 * inh_coef);
    connect_fixed_outdegree(OM4_3_2, OM4_2_E_2, 1, -70 * inh_coef);
    connect_fixed_outdegree(OM4_3_2, OM4_2_F_2, 1, -3 * inh_coef);
	// output to OM4
	connect_fixed_outdegree(OM4_2_F_1, OM5_2_F, 4, 30);
    connect_fixed_outdegree(OM4_2_F_2, OM5_2_F, 4, 30);
	// output to IP
	connect_fixed_outdegree(OM4_2_E_1, eIP_E, 2, 7, neurons_in_ip);
    connect_fixed_outdegree(OM4_2_E_2, eIP_E, 2, 7, neurons_in_ip);
    connect_fixed_outdegree(OM4_2_F_1, eIP_F, 4, 5, neurons_in_ip);
    connect_fixed_outdegree(OM4_2_F_2, eIP_F, 4, 5, neurons_in_ip);
	...
	connect_fixed_outdegree(iIP_E, OM4_2_F_1, 0.5, -0.1, neurons_in_ip);
    connect_fixed_outdegree(iIP_E, OM4_2_F_2, 0.5, -0.1, neurons_in_ip);
...
//	}
	...
	for (unsigned int sim_iter = 0; sim_iter < SIM_TIME_IN_STEPS; sim_iter++) {
		...
		if (activated_C_ == 0) {
			...
			if (local_iter != 0 && (local_iter + 200) % steps_activation_C0 == 0) {
			...
			if (local_iter != 0 && (local_iter + 200) % steps_activation_C1 == 0) {
			...
			if ((begin_C_spiking[1] <= shifted_iter_time) && (shifted_iter_time < end_C_spiking[1])) {
				...
				if ((begin_C_spiking[2] <= shifted_iter_time) && (shifted_iter_time < end_C_spiking[2])) {
					...
					if ((begin_C_spiking[3] <= shifted_iter_time) && (shifted_iter_time < end_C_spiking[3])) {
                        ...
                        // CV4.2
                        if ((begin_C_spiking[4] <= shifted_iter_time) && (shifted_iter_time < end_C_spiking[3])) {
                            decrease_lvl_Ia_spikes = 1;
                        } else {
                            // CV5
                            if ((begin_C_spiking[5] <= shifted_iter_time) && (shifted_iter_time < end_C_spiking[4])) {
                                decrease_lvl_Ia_spikes = 2;
                            }
                        }
					}
