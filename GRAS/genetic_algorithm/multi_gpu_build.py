...
class Build:
    ...
    @staticmethod
    def create_build_string(individual, number):
        return ' ' + ' '.join(map(str, individual.gen)) + ' ' + str(number)

    @staticmethod
    def build(individual, itest):
        cmd_run = f"{Build.script_place}/{Build.buildname}" + Build.create_build_string(individual, itest)
