...
def draw_slice_borders(sim_time, skin_stim_time):
	a = [6 * skin_stim_time]
	for i in range(1, 20):
		a.append(a[i - 1] + (6 * skin_stim_time if i % 2 == 0 else 5 * skin_stim_time) )
	for i in a:
		plt.axvline(x=i, linewidth=3, color='k')
	for i in range(0, sim_time, 125):
...
def plot(skin_stim_time, names, voltages, g_exc, g_inh, spikes, step, save_to, plot_only=None):
	for name, voltage, g_e, g_i, s in zip(names, voltages, g_exc, g_inh, spikes):
		...
		plt.xticks(range(0, sim_time + 1, skin_stim_time), [""] * sim_time, color='w')
		...
		plt.ylabel("Spikes, n")
		plt.ylim(bottom=0)
		plt.xticks(range(0, sim_time + 1, skin_stim_time), range(0, sim_time + 1, skin_stim_time),
		           fontsize=8)
...
def run():
	...
	folder_for_saving_results = "/home/yuliya/Desktop/STDP/GRAS/dat/results"

	if not os.path.isdir(folder_for_saving_results):
		os.mkdir(folder_for_saving_results)
		logger.info("created folder results")
	...
	if len(sys.argv) == 2:
		...
		path = "/home/yuliya/Desktop/STDP/GRAS/dat"
