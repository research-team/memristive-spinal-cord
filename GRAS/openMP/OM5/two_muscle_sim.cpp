...
void connect_one_to_all(const Group& pre_neurons,
                        const Group& post_neurons, float syn_delay,
                        float weight) {
...
void init_network(float inh_coef, int pedal, int has5ht) {
	...
	/// groups of neurons
	Group EES = form_group("EES", 20);
	Group E1 = form_group("E1", 20);
	Group E2 = form_group("E2", 20);
	Group E3 = form_group("E3", 20);
	Group E4 = form_group("E4", 20);
	Group E5 = form_group("E5", 20);
	...
	/// connectomes
	connect_fixed_outdegree(EES, E1, 1, 500, syn_outdegree);
	connect_fixed_outdegree(E1, E2, 1, 200, syn_outdegree);
	connect_fixed_outdegree(E2, E3, 1, 200, syn_outdegree);
	connect_fixed_outdegree(E3, E4, 1, 200, syn_outdegree);
	connect_fixed_outdegree(E4, E5, 1, 200, syn_outdegree);
...
//     connect_one_to_all(CV1, OM1_0, 1.5, 0.2 * quadru_coef * sero_coef);//02
//     connect_one_to_all(CV2, OM1_0, 1, 0.25 * quadru_coef * sero_coef);//01
...
//     connect_one_to_all(CV4, OM1_3, 0.5, 0.5);
//     connect_one_to_all(CV5, OM1_3, 0.5, 0.9);
//     // E1
//     connect_fixed_outdegree(E1, OM1_0, 1, 100); // 90
//     // inner connectomes
//     connect_fixed_outdegree(OM1_0, OM1_1, 0.2, 5);//6
//     connect_fixed_outdegree(OM1_1, OM1_2_E, 0.5, 0.0009 );//*2
//     connect_fixed_outdegree(OM1_1, OM1_2_F, 1, 0.0009);
//     connect_fixed_outdegree(OM1_1, OM1_3, 1, 0.009); // 0.9
//     connect_fixed_outdegree(OM1_2_E, OM1_1, 2.5, 0.000000000000001);//00000003
//     connect_fixed_outdegree(OM1_2_F, OM1_1, 2.5, 0.000000000000001);//00000001
//     connect_fixed_outdegree(OM1_2_E, OM1_3, 1, 0.000009 );//*4
//     connect_fixed_outdegree(OM1_2_F, OM1_3, 1, 0.000008 );//*2
//     connect_fixed_outdegree(OM1_3, OM1_1, 0.3, -0.3 * inh_coef);
...
//     // output to IP
//     connect_fixed_outdegree(OM1_2_E, eIP_E, 1.5, 0.08 * 3, neurons_in_ip);
// //    connect_fixed_outdegree(OM1_2_F, eIP_F, 4, 0.009 * 6, neurons_in_ip);
	...
	// input from sensory [CV]
	connect_one_to_all(CV5, OM5_0, 0.5, 0.4 * quadru_coef * sero_coef);
	// input from sensory [CD]
	connect_one_to_all(CD5, OM5_0, 1, 50);
	// inner connectomes
	connect_fixed_outdegree(OM5_0, OM5_1, 1, 100);
	connect_fixed_outdegree(OM5_1, OM5_2_E, 1, 1);
	connect_fixed_outdegree(OM5_1, OM5_2_F, 1, 1);
	connect_fixed_outdegree(OM5_1, OM5_3, 1, 3);
	connect_fixed_outdegree(OM5_2_E, OM5_1, 2.5, 0.0001);
	connect_fixed_outdegree(OM5_2_F, OM5_1, 2.5, 0.0001);
	connect_fixed_outdegree(OM5_2_E, OM5_3, 1, 3);
	connect_fixed_outdegree(OM5_2_F, OM5_3, 1, 3);
	connect_fixed_outdegree(OM5_3, OM5_1, 1, -0.00002 * inh_coef);
	connect_fixed_outdegree(OM5_3, OM5_2_E, 1, -0.001 * inh_coef);
	connect_fixed_outdegree(OM5_3, OM5_2_F, 1, -0.001 * inh_coef);
	// output to IP
...
//	rand_normal_init_array<float>(nrn_c_m, neurons_number, 200, 6); // membrane capacity (185, 215)
	...
	for (unsigned int sim_iter = 0; sim_iter < SIM_TIME_IN_STEPS; sim_iter++) {
		...
		for (int tid = 0; tid < neurons_number; tid++) {
			...
			if (tid <= 19 && EES_activated) nrn_has_spike[tid] = true;
			// iIP_F
			if (C0_activated && C0_early_activated && 2725 <= tid && tid <= 2920 && (sim_iter % 10 == 0))
				nrn_has_spike[2725 + static_cast<int>(neurons_in_ip * standard_uniform(generator))] = true;
			// skin stimulations
			if (!C0_activated) {

				if (tid == 120 && CV1_activated && standard_uniform(generator_s) >= 0.5) nrn_has_spike[tid] = true;
				if (tid == 121 && CV2_activated && standard_uniform(generator_s) >= 0.5) nrn_has_spike[tid] = true;
				if (tid == 122 && CV3_activated && standard_uniform(generator_s) >= 0.5) nrn_has_spike[tid] = true;
				if (tid == 123 && CV4_activated && standard_uniform(generator_s) >= 0.5) nrn_has_spike[tid] = true;
				if (tid == 124 && CV5_activated && standard_uniform(generator_s) >= 0.5) nrn_has_spike[tid] = true;
			}
		...
		for (int tid = 0; tid < synapses_number; tid++) {
