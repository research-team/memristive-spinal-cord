...
void neurons_kernel(const float *C_m,
	...
	if (tid < neurons_number) {
		...
		if (activated_C_ == 0 && early_activated_C_ == 0 && 2327 <= tid && tid <= 2522 && (sim_iter % 10 == 0)) {
			...
			has_spike[2327 + static_cast<int>(196 * curand_uniform(&localState))] = true;
		}
		// Skin stimulations
		if (activated_C_ == 1) {
			...
			// CV4_2
            if (tid == 124 && (begin_C_spiking[3] + cv_offset) < shifted_sim_iter && shifted_sim_iter < (end_C_spiking[3] + cv_offset) && curand_uniform(&localState) >= 0.5) {
                has_spike[tid] = true;
            }
			// CV5_1
			if (tid == 125 && begin_C_spiking[4] < shifted_sim_iter && shifted_sim_iter < end_C_spiking[4] && curand_uniform(&localState) >= 0.5) {
				has_spike[tid] = true;
			}
			// CV5_2
//            if (tid == 126 && (begin_C_spiking[4] + cv_offset) < shifted_sim_iter && shifted_sim_iter < (end_C_spiking[4] + cv_offset) && curand_uniform(&localState) >= 0.5) {
//                has_spike[tid] = true;
//            }
		}
...
void init_network(float inh_coef, int pedal, int has5ht) {
	...
	Group CV1_1 = form_group("CV1_1", 1);
    Group CV2_1 = form_group("CV2_1", 1);
	Group CV3_1 = form_group("CV3_1", 1);
	Group CV4_1 = form_group("CV4_1", 1);
    Group CV4_2 = form_group("CV4_2", 1);
	Group CV5_1 = form_group("CV5_1", 1);
    Group CV5_2 = form_group("CV5_2", 1);
	...
	connect_one_to_all(CV1_1, iIP_E, 0.5, 50);
	connect_one_to_all(CV2_1, iIP_E, 0.5, 50);
	connect_one_to_all(CV3_1, iIP_E, 0.5, 50);
	connect_one_to_all(CV4_1, iIP_E, 0.5, 50);
	connect_one_to_all(CV5_1, iIP_E, 0.5, 50);

	/// OM 1
	// input from EES group 1
	connect_fixed_outdegree(E1, OM1_0, 2, 15);
	// input from sensory
	connect_one_to_all(CV1_1, OM1_0, 0.5, 10 * quadru_coef * sero_coef);
	connect_one_to_all(CV2_1, OM1_0, 0.5, 10 * quadru_coef * sero_coef);
	// [inhibition]
	connect_one_to_all(CV3_1, OM1_3, 1, 80);
	connect_one_to_all(CV4_1, OM1_3, 1, 80);
	connect_one_to_all(CV5_1, OM1_3, 1, 80);
	// inner connectomes
	...
	connect_one_to_all(CV2_1, OM2_0, 0.5, 10.5 * quadru_coef * sero_coef);
	connect_one_to_all(CV3_1, OM2_0, 0.5, 10.5 * quadru_coef * sero_coef);
	// [inhibition]
	connect_one_to_all(CV4_1, OM2_3, 1, 80);
	connect_one_to_all(CV5_1, OM2_3, 1, 80);
	// inner connectomes
	...
	// input from sensory [CV]
	connect_one_to_all(CV3_1, OM3_0, 0.5, 10.5 * quadru_coef * sero_coef);
	connect_one_to_all(CV4_1, OM3_0, 0.5, 10.5 * quadru_coef * sero_coef);
	// [inhibition]
	connect_one_to_all(CV5_1, OM3_3, 1, 80);
	...
	connect_one_to_all(CV5_1, OM4_0_1, 0.5, 10.5 * quadru_coef * sero_coef);
	...
	// output to IP
	connect_fixed_outdegree(OM4_2_E_1, eIP_E, 2, 7 / 2, neurons_in_ip);
	connect_fixed_outdegree(OM4_2_F_1, eIP_F, 4, 5 / 2, neurons_in_ip);
    connect_fixed_outdegree(OM4_2_E_2, eIP_E, 2, 7 / 2, neurons_in_ip);
    connect_fixed_outdegree(OM4_2_F_2, eIP_F, 4, 5 / 2, neurons_in_ip);

	/// OM 5
	// input from EES group 5
	connect_fixed_outdegree(E5, OM5_0, 1, 7);
	// input from sensory [CV]
	connect_one_to_all(CV5_1, OM5_0, 0.5, 10.5 * quadru_coef * sero_coef);
	// input from sensory [CD]
