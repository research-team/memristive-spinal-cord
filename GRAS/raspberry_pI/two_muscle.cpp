...
void init_network() {
	...
	/// connectomes
	connect_fixed_outdegree(EES, E1, 1, 10000, syn_outdegree, true);
	connect_fixed_outdegree(E1, E2, 1, 10000, syn_outdegree, true);
	connect_fixed_outdegree(E2, E3, 1, 10000, syn_outdegree, true);
	connect_fixed_outdegree(E3, E4, 1, 10000, syn_outdegree, true);
	connect_fixed_outdegree(E4, E5, 1, 10000, syn_outdegree, true);
...
void simulate(int cms, int ees) {
	...
	init_array<float>(V_m, neurons_number, 20800);
	...
	auto *syn_weight = (float *) malloc(datasize<float>(synapses_number));
	...
	for (unsigned int sim_iter = 0; sim_iter < SIM_TIME_IN_STEPS; sim_iter++) {
		...
		for(unsigned int tid = 0; tid < neurons_number; tid++) {
			...
			if (C0_activated && C0_early_activated && 952 <= tid && tid <= 1001) {
				nrn_has_spike[952 + static_cast<int>(neurons_in_ip * standard_uniform(generator))] = true;
			}
			// skin stimulations
			if (!C0_activated) {
		...
		for(unsigned int tid = 0; tid < synapses_number; tid++) {
			...
			if (syn_delay_timer[tid] == 0) {
				...
				if (V_m[syn_post_nrn_id[tid]] > syn_weight[tid] && V_m[syn_post_nrn_id[tid]] < 65000)
					#pragma omp atomic
					V_m[syn_post_nrn_id[tid]] += syn_weight[tid];
