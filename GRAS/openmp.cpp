...
void copy_data_to(GroupMetadata &metadata, const float* nrn_v_m, const float* nrn_g_exc, const float* nrn_g_inh,
	              const bool *nrn_has_spike, unsigned int sim_iter) {
...
void simulate(int cms, int ees, int inh, int ped, int ht5, int save_all, int itest) {
	...
	for (unsigned int sim_iter = 0; sim_iter < SIM_TIME_IN_STEPS; sim_iter++) {
		...
		for (int tid = 0; tid < neurons_number; tid++) {
			...
			// iIP_F
			if (C0_activated && C0_early_activated && 2225 <= tid && tid <= 2420 && (sim_iter % 10 == 0))
				nrn_has_spike[2225 + static_cast<int>(neurons_in_ip * standard_uniform(generator))] = true;
			// skin stimulations
			if (!C0_activated) {
				if (tid == 120 && CV1_activated && standard_uniform(generator) >= 0.5) nrn_has_spike[tid] = true;
				if (tid == 121 && CV2_activated && standard_uniform(generator) >= 0.5) nrn_has_spike[tid] = true;
				if (tid == 122 && CV3_activated && standard_uniform(generator) >= 0.5) nrn_has_spike[tid] = true;
				if (tid == 123 && CV4_activated && standard_uniform(generator) >= 0.5) nrn_has_spike[tid] = true;
				if (tid == 124 && CV5_activated && standard_uniform(generator) >= 0.5) nrn_has_spike[tid] = true;
			}
			...
			// ionic currents
			float I_NA = g_Na * pow(nrn_m[tid], 3) * nrn_h[tid] * (nrn_v_m[tid] - E_Na);
			float I_K = g_K * pow(nrn_n[tid], 4) * (nrn_v_m[tid] - E_K);
			float I_L = g_L * (nrn_v_m[tid] - E_L);
