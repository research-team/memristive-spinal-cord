...
def run():
	...
	if len(sys.argv) == 2:
		...
		t = time.ctime()
		pathnew = f"/home/kseniia/Desktop/OM1-2/8_JAN/{t}"
		os.makedirs(pathnew)
		...
		target = f"/home/kseniia/Desktop/OM1-2/8_JAN/{t}"
		copy(source, target)

	#plot(skin_stim_time, *read_data(path), step=step, save_to=f"{path}/OM5/results/", plot_only=nuclei) #must change path! /OMx/
	plot(skin_stim_time, *read_data(path), step=step, save_to=f"/home/kseniia/Desktop/OM1-2/8_JAN/{t}", plot_only=nuclei) #must change path! /OMx/
