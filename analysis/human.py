...
def lowfilter(signal, N = 2, Wn = 0.05):
...
def draw_slices(start, end, titles, time, period, muscle, filer = False):
    ...
    for s, e, t in zip(start, end, titles):
        ...
        if filter:
            d_f = lowfilter(np.array(d))
            d_p = d_f-d
            d_b = butter_bandpass_filter(np.array(d_f), lowcut, highcut, fs)
        ...
        for i in range(10):
            ...
            # plt.plot(np.arange(len(p)) * 0.25, p)
            p = d_b[time*4+i*period*4:time*4+(i+1)*period*4] + slice_height *i
            plt.plot(np.arange(len(p)) * 0.25, p)
...
mat_contents = sio.loadmat('/Users/sulgod/Downloads/humandata_new/6v/17062021 2+5- 210ms 20hz 10.2ma  walk 1.mat')
...
for i in range(1):
    ...
    k = 0
    yticks = []
    # draw_channels(start, end, titles, filer = True)
    draw_slices(start, end, titles, start_time, period, muscle_channel, filer = True)
