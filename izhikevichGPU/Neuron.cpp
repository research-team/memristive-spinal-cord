...
public:
	...
	void addMultimeter() {
		...
		membrane_potential = new float[sim_time_step];
		// allocate memory for recording I
		current_potential = new float[sim_time_step];
	}

	void addSpikedetector() {
		// set flag that this neuron has the multimeter
		has_spikedetector = true;
		// allocate memory for recording spikes
		spike_times = new float[ sim_time_step / ref_time_step ];
	}
	...
	void set_ref_t(float ref_t){
		this->ref_time_step = ms_to_step(ref_t);
	}

	int get_ref_t(){
		return this->ref_time_step;
	}
...
private:
	// Object variables
	int id{};							// neuron ID
	int sim_time_step{};				// simulation time in steps
	...
	float *current_potential{};			// array of membrane potential values

	// Stuff variables
	const float current_tau = 6.0;		// step of current decreasing/increasing
	const int steps_in_ms = 10;			// how much steps in 1 ms
