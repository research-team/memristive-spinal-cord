...
class NeuralNetwork:
    ...
    def runSimulation(self, frequency=40, name="", amplitude="optimal"):
        ...
        if frequency > 0:
            ...
            if self.h.PcID == 0:
                ...
                print "\t{:.2f}".format(self.percFibersIa_GM * 100) + "% of GM Ia fibers, " + "{:.2f}".format(
                    self.percFibersII_GM * 100) + "% of GM II fibers and " + "{:.2f}".format(
                    self.percMn_GM * 100) + "% of GM Mn receive EES "
    ...
    def apListToMatrix(self, cellType, apList):
        ...
        self.comm.gather(nCellxHost, tot_nCellxHost)
    ...
    def synth_EMG(self, firings, nSec):  # AP in ms
        ...
        for i in range(firings.shape[0]):
            ...
            amplitudeMod = (1 - (np.linspace(0, 1, nS[i]) ** 2)) * np.concatenate(
        ...
        for jj in currAmp:
            ...
            if (self.h.PcID == 0):
                ...
                logging.info("Computing the response: "
                             "{:.2f}".format(percFibersIa * 100) + "%, "
                             "{:.2f}".format(percFibersII * 100) + "%, "
                             "{:.2f}".format(percMn * 100) + "%."
                )
            ...
            if (self.h.PcID == 0):
                ...
                IndexEES_Iaf = np.concatenate((np.nonzero(Global_AP_Iaf[:, 0]), np.nonzero(Global_AP_Iaf[:, 1])),
                                              axis=1)
                ...
                if IndexEES_Mn.size > 0:
                    ...
                    IndexMn_Early = np.extract(IndexEES_Mn < (EESpulseTime + 3.5) / dt,
                                               IndexEES_Mn)  # first EES pulse at 10 ms and second at 135 (15th ms recorded)
                ...
                logging.info("Amplitudes calculated")
