...
#define gmax _p[0]
#define e _p[1]
#define risetime _p[2]
#define decaytime _p[3]
#define taudgaba _p[4]
#define decaygaba _p[5]
#define xloc _p[6]
#define yloc _p[7]
#define tag1 _p[8]
#define tag2 _p[9]
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 pnt_receive_size[_mechtype] = 1;
 	...
 	ivoc_help("help ?1 GABAA_S /Users/sulgod/msc/reflex_arc/neuron/many_connection/x86_64/gaba.mod\n");
...
{  double* _p; Datum* _ppvar; Datum* _thread; _NrnThread* _nt;
 ...
 _tsav = t; {
    ...
    double __state = R;
    double __primary = (R + factor * _args[0] * ( dgaba ) ) - __state;
     __primary += ( 1. - exp( 0.5*dt*( ( - 1.0 ) / risetime ) ) )*( - ( 0.0 ) / ( ( - 1.0 ) / risetime ) - __primary );
    R += __primary;
  } else {
 R = R + factor * _args[0] * ( dgaba )  ;
     }
     if (nrn_netrec_state_adjust && !cvode_active_){
    /* discon state adjustment for cnexp case (rate uses no local variable) */
    double __state = D;
    double __primary = (D + factor * _args[0] * ( dgaba ) ) - __state;
     __primary += ( 1. - exp( 0.5*dt*( ( - 1.0 ) / decaytime ) ) )*( - ( 0.0 ) / ( ( - 1.0 ) / decaytime ) - __primary );
    D += __primary;
  } else {
 D = D + factor * _args[0] * ( dgaba )  ;
     }
     if (nrn_netrec_state_adjust && !cvode_active_){
    /* discon state adjustment for cnexp case (rate uses no local variable) */
