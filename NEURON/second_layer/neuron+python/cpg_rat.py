...
class CPG:
    def __init__(self, speed, ees_fr, inh_p, step_number, layers, extra_layers, N):
        ...
        # for layer in range(1, layers):
        #     connectcells(self.dict_2F[layer - 1], self.dict_2F[layer], 0.0005, 2)
        #
        # for layer in range(layers, extra_layers):
        #     connectcells(self.dict_2F[layer - 1], self.dict_2F[layer], 0.0005, 2)
        #
        # for layer in range(layers):
        #     connectcells(self.dict_1[layer], self.dict_2F[layer], 0.0005, 3)
        #     connectcells(self.dict_2F[layer], self.dict_1[layer], 0.0005, 4)
        #
        # for layer in range(layers, extra_layers):
        #     connectcells(self.dict_1[layer], self.dict_2F[layer], 0.0005, 3)
        #     connectcells(self.dict_2F[layer], self.dict_1[layer], 0.0005, 4)
        #
        # connectcells(self.dict_CV[0], self.OM1_0F, 0.00001, 3)
        # connectcells(self.OM1_0F, self.dict_1[0], 0.00005, 3)
        ...
        connectcells(self.dict_CV[0], self.OM1_0E, 0.00037, 2)
        for layer in range(1, layers):
            connectcells(self.dict_CV[layer], self.dict_0[layer], 0.00038, 2)
        ...
        genconnect(self.ees, self.Ia_aff_E, 0.5, 0.5)
        genconnect(self.ees, self.Ia_aff_F, 0.5, 0.5)
        ...
        connectcells(self.Ia_aff_E, self.mns_E, 0.5, 0.5)
        connectcells(self.Ia_aff_F, self.mns_F, 0.5, 0.5)
        ...
        for layer in range(layers):
            ...
            connectcells(self.dict_2E[layer], self.dict_IP_E[layer], 1.05, 2)
            ...
            # connectcells(self.dict_2F[layer], self.dict_IP_F[layer], 0.00015, 2)
            # connectcells(self.dict_IP_F[layer], self.mns_F, 0.00015, 2)
        ...
        '''C5'''
        connectcells(self.dict_CV_1[5], self.dict_0[4], 0.00025*k*speed, 2)
...
def connectinsidenucleus(nucleus):
    connectcells(nucleus, nucleus, 0.35, 0.5)
...
def spikeout(pool, name, version, v_vec):
    ...
    if rank == 0:
        ...
        with hdf5.File('./res/AIR_{}_speed_{}_layers_{}.hdf5'.format(name, speed, layers), 'w') as file:
          for i in range(step_number):
