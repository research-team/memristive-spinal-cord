...
class CPG:
    def __init__(self, speed, ees_fr, inh_p, step_number, layers, extra_layers, N):
        ...
        for layer in range(1, 5):
            connectcells(self.dict_IP_E[layer-1], self.dict_IP_E[layer], 0.35*layer, 2)
            connectcells(self.dict_IP_F[layer-1], self.dict_IP_F[layer], 0.35*layer, 2)
        for layer in range(layers):
            '''Extensor'''
            ...
            if layer > 3:
                connectcells(self.dict_IP_E[layer], self.Ia_aff_E, layer*0.00037, 2, True)
            else:
