...
class CPG:
    def __init__(self, speed, ees_fr, inh_p, step_number, layers, extra_layers, N):
        ...
        #     connectcells(self.dict_IP_E[layer-1], self.dict_IP_E[layer], 0.25*layer, 2)
        #     connectcells(self.dict_IP_F[layer-1], self.dict_IP_F[layer], 0.25*layer, 2)
        for layer in range(layers):
            '''Extensor'''
...
def connectinsidenucleus(nucleus):
    connectcells(nucleus, nucleus, 0.4, 1)
