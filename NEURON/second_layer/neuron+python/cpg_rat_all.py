...
'''
class CPG:
    def __init__(self, speed, ees_fr, inh_p, step_number, layers, extra_layers, N, starttime, starttimeCV):
        ...
        self.muscle_E = self.addpool(nMN*150, "muscle_E", "muscle")
        ...
        self.Iagener_E = self.addIagener(self.muscle_E, self.muscle_F, (starttimeCV + 10))
        self.Iagener_F = self.addIagener(self.muscle_F, self.muscle_F, (starttimeCV + speed*6))
        ...
        for layer in range(layers+1):
            ...
            for i in range(step_number):
                self.dict_C[layer].append(self.addgener(25 + starttime + starttimeCV + speed * layer + i * (speed * (layers + 1) + 125), random.gauss(cfr, cfr/10), (speed / c_int + 1)))

        for layer in range(layers, extra_layers):
            self.dict_C[layer] = []
            for i in range(step_number):
                self.dict_C[layer].append(self.addgener(25 + starttime + starttimeCV + speed * (layer - 5) + int(speed * 0.3) + i * (speed * 6 + 125), random.gauss(cfr, cfr/10), speed / c_int))
        ...
        for i in range(step_number):
            self.C_0.append(self.addgener(25 + starttime + starttimeCV + speed * 6 + i * (speed * 6 + 125), cfr, 125/c_int, False))
            # self.V0v.append(self.addgener(25 + starttime + starttimeCV + speed * 4 + i * (speed * 6 + 125), 100, int(125/10), False))
        ...
        for layer in range(1, layers):
            connectcells(self.dict_2F[layer - 1], self.dict_2F[layer], 1.85, 3.5)
        ...
        connectcells(self.dict_CV[0], self.OM1_0F, 0.0005, 3)
        # connectcells(self.V0v, self.dict_2F[0], 1.85, 3)
        ...
        for layer in range(1, layers):
            connectcells(self.dict_CV[layer], self.dict_0[layer], 0.00043, 2)
        ...
        for layer in range(2, layers+1):
            if layer > 3:
                for i in range(0, (layer - 2)):
                    connectcells(self.dict_C[layer], self.dict_3[i], 1.5, 1)
        ...
        genconnect(self.ees, self.dict_CV[0], 0.15, 2)
        genconnect(self.Iagener_E, self.Ia_aff_E, 0.001, 1, False, 15)
        genconnect(self.Iagener_F, self.Ia_aff_F, 0.001, 1, False, 15)

        connectcells(self.Ia_aff_E, self.mns_E, 1.75, starttime)
        connectcells(self.Ia_aff_F, self.mns_F, 1.5, starttime)
        ...
        for layer in range(1, 4):
            ...
            connectcells(self.dict_IP_F[layer-1], self.dict_IP_F[layer+1], 0.65*layer, 1)
        for layer in range(layers):
            '''Extensor'''
            ...
            connectcells(self.dict_2E[layer], self.dict_IP_E[layer], 2.15, 2)
            connectcells(self.dict_IP_E[layer], self.mns_E, 3.15, 3)
            if layer > 3:
                connectcells(self.dict_IP_E[layer], self.Ia_aff_E, layer*0.00025, 1, True)
            else:
            ...
            connectcells(self.dict_IP_F[layer], self.mns_F, 2.99, 2)
            connectcells(self.dict_IP_F[layer], self.Ia_aff_F, 0.0005, 1, True)

        for layer in range(layers+1):
            '''skin inputs'''
            connectcells(self.dict_C[layer], self.dict_CV_1[layer], 0.025*k*speed, 1)
        ...
        connectcells(self.dict_CV_1[2], self.dict_0[2], 0.0004*k*speed, 3)
        connectcells(self.dict_CV_1[2], self.dict_0[3], 0.0002*k*speed, 3)
        connectcells(self.dict_CV_1[2], self.dict_0[4], 0.0001*k*speed, 3)
        #
        '''C4'''
        connectcells(self.dict_CV_1[3], self.dict_0[2], 0.00035*k*speed, 3)
        connectcells(self.dict_CV_1[3], self.dict_0[3], 0.00032*k*speed, 3)
        connectcells(self.dict_CV_1[4], self.dict_0[2], 0.00035*k*speed, 3)
        connectcells(self.dict_CV_1[4], self.dict_0[3], 0.00032*k*speed, 3)
        ...
        '''C=1 Extensor'''
        connectcells(self.IP_E, self.iIP_E, 0.0001, 1)

        for layer in range(layers+1):
            connectcells(self.dict_CV_1[layer], self.iIP_E, 0.8, 1)
            connectcells(self.dict_C[layer], self.iIP_E, 0.08, 1)
        ...
        connectcells(self.iIP_E, self.Ia_aff_F, 1.5, 1, True)
        connectcells(self.iIP_E, self.mns_F, 1.5, 1, True)
        ...
        connectcells(self.iIP_F, self.Ia_aff_E, 1.5, 1, True)
        connectcells(self.iIP_F, self.mns_E, 1.4, 1, True)
        connectcells(self.C_0, self.iIP_F, 0.8, 1)

        '''reflex arc'''
        connectcells(self.iIP_E, self.Ia_E, 0.001, 1)
        connectcells(self.IP_E, self.Ia_E, 0.08, 1)
        ...
        connectcells(self.Ia_E, self.iIP_F, 0.2, 1, True)
        ...
        connectcells(self.IP_F, self.Ia_F, 0.05, 1)
...
def spikeout(pool, name, version, v_vec):
    ...
    if rank == 0:
        ...
        with hdf5.File('./res/PLT_rat4_{}_speed_{}_layers_{}_eeshz_{}.hdf5'.format(name, speed, layers, ees_fr), 'w') as file:
            for i in range(step_number):
                sl = slice((0+i*one_step_time*40),(1000+(i+1)*one_step_time*40 + 1000*5))
...
if __name__ == '__main__':
    ...
    cpg_ankle_L = CPG(speed, ees_fr, 100, step_number, layers, extra_layers, N, 2, speed*6)
    cpg_ankle_R = CPG(speed, ees_fr, 100, step_number, layers, extra_layers, N, 2, 2)
    cpg_knee_L = CPG(speed, ees_fr, 100, step_number, layers, extra_layers, N, 1, speed*7)
    cpg_knee_R = CPG(speed, ees_fr, 100, step_number, layers, extra_layers, N, 1, speed)

    cpgs.append((cpg_ankle_L, "cpg_ankle_L"))
    cpgs.append((cpg_ankle_R, "cpg_ankle_R"))
    cpgs.append((cpg_knee_L, "cpg_knee_L"))
    cpgs.append((cpg_knee_R, "cpg_knee_R"))
    ...
    for cpg in cpgs:
        rec = []
        for group in cpg[k_nrns].motogroups:
            rec.append(spike_record(group[k_nrns], True))
        motorecorders.append(rec)

    logging.info(f'len rec {len(rec)}')
    logging.info(f'len motorec {len(motorecorders)}')
    logging.info(f'motorec {motorecorders}')

    for cpg in cpgs:
        rec = []
        for group in cpg[k_nrns].motogroups:
            rec.append(spike_record(group[k_nrns]))
        motorecorders_mem.append(rec)
    # affrecorders = []
    ...
    # i = 0
    for cpg, motorecorder in zip(cpgs, motorecorders):
        # logging.info(f'motorecorder {motorecorder}')
        for group, recorder in zip(cpg[k_nrns].motogroups, motorecorder):
            spikeout(group[k_nrns], f'{group[k_name]}_{cpg[k_name]}', 0, recorder)
            # i=+1

    # j = 0
    for cpg, motorecorder in zip(cpgs, motorecorders_mem):
        # logging.info(f'j {j}')
        for group, recorder in zip(cpg[k_nrns].motogroups, motorecorder):
            spikeout(group[k_nrns], f'mem_{group[k_name]}_{cpg[k_name]}', 0, recorder)
        # j=+1
