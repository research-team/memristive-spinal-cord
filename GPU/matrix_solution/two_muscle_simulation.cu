...
const float g_bar = 1500;

__global__
void GPU_neurons_kernel(
		float *V_m,
		float *h,
		float *m,
		float *n,
...
//		}
		...
		float prev_V = V_m[tid];

		const double V = V_m[tid] - V_adj;

		// Transition rates between open and closed states of the potassium channels
		float alpha_n = 0.032 * (15.0 - V) / (std::exp((15.0 - V) / 5.0) - 1.0); // 0.02 * (V_m[tid] - 25) / (1 - std::exp(-(V_m[tid] - 25) / 9));
		float beta_n = 0.5 * std::exp((10.0 - V) / 40.0); // -0.002 * (V_m[tid] - 25) / (1 - std::exp((V_m[tid] - 25) / 9));

		// Transition rates between open and closed states of the activation of sodium channels
		float alpha_m = 0.32 * (13.0 - V) / (std::exp((13.0 - V) / 4.0) - 1.0); // 0.182 * (V_m[tid] + 35) / (1 - std::exp(-(V_m[tid] + 35) / 9));
		float beta_m = 0.28 * (V - 40.0) / (std::exp((V - 40.0) / 5.0) - 1.0); // -0.124 * (V_m[tid] + 35) / (1 - std::exp((V_m[tid] + 35) / 9));

		//Transition rates between open and closed states of the inactivation of sodium channels
		float alpha_h = 0.128 * std::exp((17.0 - V) / 18.0); // 0.25 * std::exp(-(V_m[tid] + 90) / 12);
		float beta_h = 4.0 / (1.0 + std::exp((40.0 - V) / 5.0)); // 0.25 * std::exp((V_m[tid] + 62) / 6) / std::exp((V_m[tid] + 90) / 12);

		if (tid == 200) {
			printf("[step %d] [id 200] n = %f dn1 = %f dn2 = %f \n",
					sim_iter, n[tid],
				    (alpha_n * (1 - n[tid]) - beta_n * n[tid]) * SIM_STEP,
					(alpha_n - ( alpha_n + beta_n ) * n[tid]) * SIM_STEP);
		}

		// Activation variables
		n[tid] += (alpha_n - ( alpha_n + beta_n ) * n[tid]) * SIM_STEP;// += (alpha_n * (1 - n[tid]) - beta_n * n[tid]) * SIM_STEP;
		m[tid] += (alpha_m - ( alpha_m + beta_m ) * m[tid]) * SIM_STEP;// += (alpha_m * (1 - m[tid]) - beta_m * m[tid]) * SIM_STEP;
		h[tid] += (alpha_h - ( alpha_h + beta_h ) * h[tid]) * SIM_STEP;// += (alpha_h * (1 - h[tid]) - beta_h * h[tid]) * SIM_STEP;

		// ionic currents
		float I_NA = g_Na * m[tid] * m[tid] * m[tid] * h[tid] * (V_m[tid] - E_Na);
		float I_K = g_K * n[tid] * n[tid] * n[tid] * n[tid] * (V_m[tid] - E_K);
...
//		V_m[tid] += -(I_L + I_K + I_NA + I_syn_exc + I_syn_inh) / C_m * SIM_STEP;
		...
		// [nS] excitatory conductance
		g_exc[tid] += -g_exc[tid] / tau_syn_exc * SIM_STEP;
		// [nS] inhibitory conductance
...
void simulate(int test_index, int full_save) {
	...
	// special array to keep mean data of each neuron's membrane potential
	...
	init_array<float>(n, neurons_number, 0);
	init_array<float>(h, neurons_number, 1);
	init_array<float>(m, neurons_number, 0);
